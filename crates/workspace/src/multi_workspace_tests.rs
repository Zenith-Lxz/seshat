...
#[gpui::test]
async fn test_legacy_ai_settings_cannot_restore_sidebar(cx: &mut TestAppContext) {
    init_test(cx);
    let fs = FakeFs::new(cx.executor());
    let project = Project::test(fs, [], cx).await;
    let (multi_workspace, cx) =
        cx.add_window_view(|window, cx| MultiWorkspace::test_new(project, window, cx));
    for disable_ai in [false, true, false] {
        cx.update(|_, cx| DisableAiSettings::override_global(DisableAiSettings { disable_ai }, cx));
        multi_workspace.update_in(cx, |workspace, window, cx| {
            assert!(!workspace.sidebar_available(cx));
            assert!(workspace.multi_workspace_enabled(cx));
