...
- Start directly in the editing workspace. Vim is enabled by default and remains configurable. AI, language servers, telemetry and automatic updates are disabled by default. Save does not automatically format, trim trailing spaces or add a final newline. Per-language Prettier integration is disabled by default so merely opening Markdown does not preinstall its formatter.
